...
侧栏「工作台」入口,聚合**所有运行**的任务到六个分区(运行中/排队中/等待输入/已失败/待审查/可集成),条目点击跳转对应运行;运行级操作:暂停/继续、优先级(-5..5);排队条目展示原因(全局/项目/单类型预算满、资源高压、错峰等待、运行已暂停);顶部显示资源采样徽标(负载/磁盘/是否高压)。

工作台顶部的**恢复面板**:崩溃或强杀后重启,自动扫描——中断任务(进程已死/状态未知,可标记失败或重跑,重跑可选含被阻塞的下游)、进程仍在(等待观察,不自动接管)、孤儿 worktree/临时分支(勾选后二次确认清理,清理带路径守护)。全部动作留审计事件。
...
| **调度设置** | 三级并发预算（全局 1–20 / 单仓库 1–10 / 单 Agent 类型 1–10，实际以四级最严为准，含各 Run 自身上限）、资源高压暂缓开关、磁盘余量阈值、交互槽预留（交互任务保留 1 个全局槽） |
...
启动前体检：九项检查（CLI 路径与版本、GUI 继承 PATH、登录会话、MCP 控制面、Git 仓库、工作区与磁盘、沙箱能力、Provider 配额、数据库健康），单项超时记「无法确认」不阻塞；失败项含影响范围与建议,可单项重检;「复制诊断包」导出脱敏 JSON(家路径→~,密钥 redact)。`start_team` 遇注定失败的环境问题会在启动前拦截并指向诊断。
